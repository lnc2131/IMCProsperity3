...
PARAMS = {
    ...
    # ── JAMS Bollinger‑Band parameters ────────────────────────────────
    Product.JAMS: {
        "bb_window": 20,      # rolling‑window length (ticks)
        "bb_k_entry": 3.0,     # enter when |z| ≥ k_entry
        "bb_k_exit": 1       # flatten when |z| ≤ k_exit
    }
...
class Trader:
    def __init__(self, params = None):
        ...
        # ── JAMS Bollinger‑Band state ────────────────────────────────
        pj = self.params[Product.JAMS]
        self.jam_window    = pj["bb_window"]
        self.jam_k_entry   = pj["bb_k_entry"]
        self.jam_k_exit    = pj["bb_k_exit"]
        self.jam_pos_limit = self.LIMIT[Product.JAMS]
        # we’ll reuse self.price_history[Product.JAMS] as the rolling buffer


    def make_JAM_orders(self, depth: OrderDepth, position: int) -> List[Order]:
        """Bollinger‑Band mean‑reversion for JAMS."""
        orders: List[Order] = []

        mid = self.get_swmid(depth)               # you already have get_swmid()
        if mid is None:
            return orders

        # rolling window using the existing price_history list
        buf = self.price_history[Product.JAMS]
        buf.append(mid)
        if len(buf) > self.jam_window:
            buf.pop(0)
        if len(buf) < self.jam_window:            # warm‑up
            return orders

        mu     = float(np.mean(buf))
        sigma  = float(np.std(buf)) or 1e-9       # avoid div/0
        z      = (mid - mu) / sigma

        # target inventory based on z‑score bands
        if   z >=  self.jam_k_entry: target = -self.jam_pos_limit
        elif z <= -self.jam_k_entry: target =  self.jam_pos_limit
        elif abs(z) <= self.jam_k_exit: target = 0
        else: target = position                  # keep current
        ...
        bid_vol    = abs(depth.buy_orders[bid])
        ask_vol    = abs(depth.sell_orders[ask])

        if delta > 0:    # need to BUY
            qty = min(delta, ask_vol, self.jam_pos_limit - position)
            if qty > 0:
                orders.append(Order(Product.JAMS, ask, qty))
                logger.print(f"JAMS BUY {qty}@{ask}  z={z:.2f}")
        else:            # need to SELL
            qty = min(-delta, bid_vol, self.jam_pos_limit + position)
            if qty > 0:
                orders.append(Order(Product.JAMS, bid, -qty))
                logger.print(f"JAMS SELL {qty}@{bid} z={z:.2f}")
    ...
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        # === JAMS Bollinger‑Band MR =========================================
        if Product.JAMS in state.order_depths:
            jam_pos = state.position.get(Product.JAMS, 0)
            jam_orders = self.make_JAM_orders(state.order_depths[Product.JAMS], jam_pos)
            if jam_orders:
