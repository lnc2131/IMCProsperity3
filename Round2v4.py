...
class Trader:
    ...
    def update_ema_values(self, state: TradingState) -> None:
        """Update EMA values for basket components based on market trades"""
        # Process new trades for each component
        for product in [Product.CROISSANTS, Product.JAMS, Product.DJEMBES]:
            if product in state.market_trades and state.market_trades[product]:
                ...
                # Calculate volume-weighted average price of recent trades
                total_volume = 0
                total_price_volume = 0
                
                for trade in trades:
                    total_volume += abs(trade.quantity)
                    total_price_volume += trade.price * abs(trade.quantity)
                
                if total_volume > 0:
                    latest_price = total_price_volume / total_volume
                    
                    # Update EMA
                    if self.ema_values[product] is None:
                        self.ema_values[product] = latest_price
                    else:
                        self.ema_values[product] = (self.ema_alpha * latest_price + 
                                                  (1 - self.ema_alpha) * self.ema_values[product])
                    
                    logger.print(f"Updated {product} EMA to: {self.ema_values[product]}")
                
    def calculate_basket_ema_value(self) -> float:
        """Calculate synthetic basket value using component EMAs"""
        # Check if we have all required EMAs
        if any(v is None for v in self.ema_values.values()):
            return None
            
        # Calculate basket value using component weights
        basket_value = (
            self.ema_values[Product.CROISSANTS] * BASKET_WEIGHTS[Product.CROISSANTS] +
            self.ema_values[Product.JAMS] * BASKET_WEIGHTS[Product.JAMS] +
            self.ema_values[Product.DJEMBES] * BASKET_WEIGHTS[Product.DJEMBES]
        )
        
        return basket_value
        

    def picnic_basket_strategy(self, state: TradingState, basket_position: int) -> Dict[str, List[Order]]:
        """Implement the spread arbitrage strategy for picnic baskets using EMA values"""
        ...
        # Update EMAs with new market data
        self.update_ema_values(state)
        
        # Calculate basket values
        synthetic_value = self.calculate_basket_ema_value()
        if synthetic_value is None:
            logger.print("Insufficient EMA data - skipping trade")
            return {}
        
        # Calculate current basket price
        basket_mid = self.get_swmid(basket_order_depth)
        if basket_mid is None:
            return {}
        
        # Calculate the spread between actual and synthetic price
        ...
        if zscore >= zscore_threshold:
            # Basket is overvalued - sell basket
            if basket_position > -target_position:
                sell_size = min(
                    abs(target_position + basket_position),
                    abs(basket_order_depth.buy_orders[best_basket_bid]),
                    max_basket_position + basket_position
                )
            ...
            # Basket is undervalued - buy basket
            if basket_position < target_position:
                buy_size = min(
                    abs(target_position - basket_position),
                    abs(basket_order_depth.sell_orders[best_basket_ask]),
                    max_basket_position - basket_position
                )
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        if Product.PICNIC_BASKET1 in state.order_depths:
            ...
            # Run the basket spread strategy with full state
            basket_orders = self.picnic_basket_strategy(state, basket_position)
        ...
        # Update debug data to include EMA values
        trader_data = json.dumps({
            ...
            "ema_values": {
                "CROISSANTS": self.ema_values[Product.CROISSANTS],
                "JAMS": self.ema_values[Product.JAMS],
                "DJEMBES": self.ema_values[Product.DJEMBES]
            }
        })
