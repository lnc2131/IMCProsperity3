...
class Trader:
    def __init__(self, params = None):
        ...
        self.jam_params = self.params[Product.JAMS]
# ───────────────── DJEMBE helpers ──────────────────────────────────────────
    ...
    # ──────────────────────────────────────────────────────────────────
    def jam_fv(self, order_depth: OrderDepth, timespan: int) -> float:
        if not order_depth.sell_orders or not order_depth.buy_orders:
            return None
    
        best_ask = min(order_depth.sell_orders.keys())
        best_bid = max(order_depth.buy_orders.keys())
        filtered_ask = [p for p in order_depth.sell_orders if abs(order_depth.sell_orders[p]) >= self.jam_params["common_maker"]]
        filtered_bid = [p for p in order_depth.buy_orders if abs(order_depth.buy_orders[p]) >= self.jam_params["common_maker"]]
        ...
        self.price_history[Product.JAMS].append(mid_price)
        if len(self.price_history[Product.JAMS]) > 20:
            self.price_history[Product.JAMS].pop(0)
    ...
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        if Product.JAMS in state.order_depths:
            pos = state.position.get(Product.JAMS, 0)
            fv = self.jam_fv(state.order_depths[Product.JAMS], self.jam_params["timespan"])
            if fv is not None:
                buy_vol = sell_vol = 0
                take, buy_vol, sell_vol = self.take_orders(
                    ...
                    fv,
                    self.jam_params["take_width"],
                    pos,
                    self.jam_params["prevent_adverse"],
                    self.jam_params["adverse_volume"],
                )
                clear, buy_vol, sell_vol = self.clear_position_order(
                    Product.JAMS,
                    fv,
                    self.jam_params["clear_width"],
                    [],
                    state.order_depths[Product.JAMS],
                    pos,
                    buy_vol,
                ...
                result[Product.JAMS] = take + clear
