...
PARAMS = {
    ...
    "SPREAD": {
        "default_spread_mean": 48,  # This will be dynamically calculated
...
class Trader:
    ...
    def get_synthetic_basket_order_depth(self, order_depths: Dict[str, OrderDepth]) -> OrderDepth:
        ...
        required_products = [Product.CROISSANT, Product.JAM, Product.DJEMBE]
        ...
        jam_best_bid = max(order_depths[Product.JAM].buy_orders.keys())
        jam_best_ask = min(order_depths[Product.JAM].sell_orders.keys())
        djembe_best_bid = max(order_depths[Product.DJEMBE].buy_orders.keys())
        djembe_best_ask = min(order_depths[Product.DJEMBE].sell_orders.keys())
        
        # Calculate the implied bid and ask for the synthetic basket
        implied_bid = (
            croissant_best_bid * BASKET_WEIGHTS[Product.CROISSANT] +
            jam_best_bid * BASKET_WEIGHTS[Product.JAM] +
            djembe_best_bid * BASKET_WEIGHTS[Product.DJEMBE]
        )
        
        implied_ask = (
            croissant_best_ask * BASKET_WEIGHTS[Product.CROISSANT] +
            jam_best_ask * BASKET_WEIGHTS[Product.JAM] +
            djembe_best_ask * BASKET_WEIGHTS[Product.DJEMBE]
        )
        
        # Calculate the maximum number of synthetic baskets available
        # For bid side (how many baskets we can sell)
        croissant_bid_volume = order_depths[Product.CROISSANT].buy_orders[croissant_best_bid] // BASKET_WEIGHTS[Product.CROISSANT]
        jam_bid_volume = order_depths[Product.JAM].buy_orders[jam_best_bid] // BASKET_WEIGHTS[Product.JAM]
        djembe_bid_volume = order_depths[Product.DJEMBE].buy_orders[djembe_best_bid] // BASKET_WEIGHTS[Product.DJEMBE]
        implied_bid_volume = min(croissant_bid_volume, jam_bid_volume, djembe_bid_volume)
        
        # For ask side (how many baskets we can buy)
        croissant_ask_volume = -order_depths[Product.CROISSANT].sell_orders[croissant_best_ask] // BASKET_WEIGHTS[Product.CROISSANT]
        jam_ask_volume = -order_depths[Product.JAM].sell_orders[jam_best_ask] // BASKET_WEIGHTS[Product.JAM]
        djembe_ask_volume = -order_depths[Product.DJEMBE].sell_orders[djembe_best_ask] // BASKET_WEIGHTS[Product.DJEMBE]
    ...
    def execute_spread_orders(self, target_position: int, basket_position: int, 
        ...
        if target_position > basket_position:
            ...
            croissant_bid_price = max(order_depths[Product.CROISSANT].buy_orders.keys())
            jam_bid_price = max(order_depths[Product.JAM].buy_orders.keys())
            djembe_bid_price = max(order_depths[Product.DJEMBE].buy_orders.keys())
            
            # Create component sell orders
            result_orders[Product.CROISSANT].append(
                Order(Product.CROISSANT, croissant_bid_price, -execute_volume * BASKET_WEIGHTS[Product.CROISSANT])
            )
            result_orders[Product.JAM].append(
                Order(Product.JAM, jam_bid_price, -execute_volume * BASKET_WEIGHTS[Product.JAM])
            )
            result_orders[Product.DJEMBE].append(
                Order(Product.DJEMBE, djembe_bid_price, -execute_volume * BASKET_WEIGHTS[Product.DJEMBE])
            )
            ...
            croissant_ask_price = min(order_depths[Product.CROISSANT].sell_orders.keys())
            jam_ask_price = min(order_depths[Product.JAM].sell_orders.keys())
            djembe_ask_price = min(order_depths[Product.DJEMBE].sell_orders.keys())
            
            # Create component buy orders
            result_orders[Product.CROISSANT].append(
                Order(Product.CROISSANT, croissant_ask_price, execute_volume * BASKET_WEIGHTS[Product.CROISSANT])
            )
            result_orders[Product.JAM].append(
                Order(Product.JAM, jam_ask_price, execute_volume * BASKET_WEIGHTS[Product.JAM])
            )
            result_orders[Product.DJEMBE].append(
                Order(Product.DJEMBE, djembe_ask_price, execute_volume * BASKET_WEIGHTS[Product.DJEMBE])
            )
    ...
    def picnic_basket_strategy(self, order_depths: Dict[str, OrderDepth], basket_position: int) -> Dict[str, List[Order]]:
        """Implement the statistical arbitrage strategy for picnic baskets"""
        # Check if we have all required order depths
        required_products = [Product.PICNIC_BASKET1, Product.CROISSANT, Product.JAM, Product.DJEMBE]
        for product in required_products:
            if product not in order_depths or not order_depths[product].buy_orders or not order_depths[product].sell_orders:
        ...
        logger.print(f"Spread z-score: {zscore} (mean: {spread_mean}, std: {spread_std})")
        ...
        if zscore >= zscore_threshold:
            ...
            if basket_position > -target_position:
                logger.print(f"Z-score {zscore} > threshold {zscore_threshold} - selling basket")
                return self.execute_spread_orders(-target_position, basket_position, order_depths)
                
        elif zscore <= -zscore_threshold:
            # Spread is too low - buy basket, sell components
            if basket_position < target_position:
                logger.print(f"Z-score {zscore} < -threshold {-zscore_threshold} - buying basket")
                return self.execute_spread_orders(target_position, basket_position, order_depths)
                
        # No trade signal
        logger.print(f"No basket trade signal (z-score: {zscore}, position: {basket_position})")
        return None
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        if Product.PICNIC_BASKET1 in state.order_depths:
            ...
            # Run the basket arbitrage strategy
            basket_orders = self.picnic_basket_strategy(state.order_depths, basket_position)
            
            if basket_orders:
                # Add all basket and component orders to the result
        ...
        trader_data = json.dumps({
            ...
            "positions": {
                ...
                "CROISSANT": state.position.get(Product.CROISSANT, 0),
                "JAM": state.position.get(Product.JAM, 0),
                "DJEMBE": state.position.get(Product.DJEMBE, 0)
            },
            "basket_spread": {
