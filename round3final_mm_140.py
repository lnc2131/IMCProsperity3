...
PARAMS = {
    ...
    Product.MAGNIFICENT_MACARONS: {
        "make_edge": 0.5,       # Reduced edge to be more competitive
        "make_probability": 0.800,
        "storage_cost": 0.1,
        "csi_threshold": 60.0,   # Critical Sunlight Index - current data shows sunlight is around 65
        "sunlight_factor": 30,   # Doubled impact from sunlight for greater sensitivity
        "min_conversion_size": 8,  # Minimum position size to trigger conversion
        "take_width": 0,        # Be aggressive on arbitrage opportunities
        "long_bias": -0.2,      # Reduced penalty on long positions even further
        "aggressive_threshold": 57.0,  # Threshold for aggressive long positions
        "panic_threshold": 55.0,  # Threshold for maximum aggression
        "position_limit": 140   # Increased position limit from 75 to 140
    }
}
...
LIMIT = {
    ...
    Product.DJEMBES: 60, Product.VOLCANIC_ROCK: 400, Product.MAGNIFICENT_MACARONS: 140,
...
class Trader:
    ...
    def macarons_implied_bid_ask(
        ...
        recent_sunlight = traderObject.get("sunlight_history", [])
        recent_sunlight.append(sunlight_index)
        if len(recent_sunlight) > 50:
            recent_sunlight.pop(0)
        traderObject["sunlight_history"] = recent_sunlight

        # Dynamically infer CSI (e.g., 20th percentile)
        if len(recent_sunlight) >= 20:
            CSI = np.percentile(recent_sunlight, 20)
        else:
            CSI = params["csi_threshold"]  # fallback

        traderObject["dynamic_csi"] = CSI

        # Implied price calc
        implied_bid = observation.bidPrice - observation.exportTariff - observation.transportFees - storage_cost
        implied_ask = observation.askPrice + observation.importTariff + observation.transportFees

        # Non-linear sunlight impact
        if sunlight_index < CSI:
            sunlight_delta = max(0.1, CSI - sunlight_index)
            nonlinear_impact = sunlight_scaling * (1 - math.exp(-sunlight_delta))
            implied_ask += nonlinear_impact
            implied_bid += nonlinear_impact * 0.8
    ...
    def macarons_arb_take(
        ...
        panic_threshold = params["panic_threshold"]
        
        implied_bid, implied_ask = self.macarons_implied_bid_ask(observation, sunlight_index, params, traderObject)
    
        # Determine trading regime
        trading_regime = "normal"
        if sunlight_index < panic_threshold:
            trading_regime = "panic"
        elif sunlight_index < aggressive_threshold:
            trading_regime = "aggressive"
        elif sunlight_index < CSI:
            trading_regime = "below_csi"
            
        # Adjust trading thresholds based on regime
        buy_threshold_adj = 0
        sell_threshold_adj = 0
        buy_size_multiplier = 1.0
        sell_size_multiplier = 1.0
        
        if trading_regime == "panic":
            # In panic mode, be extremely aggressive buying, very cautious selling
            buy_threshold_adj = take_width + 3.0  # Buy even at higher prices
            sell_threshold_adj = -1.0  # Only sell at much higher prices
            buy_size_multiplier = 2.0
            sell_size_multiplier = 0.3
            # Temporarily adjust long_bias to be less punitive
            long_bias = 0.0  # Completely eliminate storage cost penalty
            logger.print(f"[MACARONS] PANIC MODE - Extreme buy bias at sunlight {sunlight_index:.2f}")
            
        elif trading_regime == "aggressive":
            # In aggressive mode, be very eager to buy, cautious to sell
            buy_threshold_adj = take_width + 2.0
            sell_threshold_adj = -0.7
            buy_size_multiplier = 1.7
            sell_size_multiplier = 0.5
            long_bias = 0.1  # Actually bias towards longs
            logger.print(f"[MACARONS] AGGRESSIVE MODE - Strong buy bias at sunlight {sunlight_index:.2f}")
            
        elif trading_regime == "below_csi":
            # In below CSI mode, be moderately aggressive
            buy_threshold_adj = take_width + 1.0
            sell_threshold_adj = -0.4
            buy_size_multiplier = 1.5
            sell_size_multiplier = 0.7
            long_bias = min(long_bias * 0.5, -0.1)
            logger.print(f"[MACARONS] ALERT MODE - Moderate buy bias at sunlight {sunlight_index:.2f}")
            
        # Even in normal conditions, be slightly biased to buy when close to thresholds
        elif sunlight_index < CSI + 2.0:
            # Almost at the threshold, start being more aggressive
            buy_threshold_adj = take_width + 0.5
            sell_threshold_adj = -0.2
            buy_size_multiplier = 1.2
            sell_size_multiplier = 0.9
            logger.print(f"[MACARONS] WATCHING - Approaching threshold at sunlight {sunlight_index:.2f}")
    
        # Available position capacity 
        max_buy_quantity = position_limit - position
        max_sell_quantity = position_limit + position
    
        # Look for arbitrage opportunities with adjusted thresholds
        for price in sorted(list(order_depth.sell_orders.keys())):
            # Buy threshold is more aggressive in lower sunlight regimes
            if price < implied_bid - take_width + buy_threshold_adj - long_bias:
                # Scale desired buy quantity based on regime
                desired_quantity = min(
                    abs(order_depth.sell_orders[price]), 
                    int(max_buy_quantity * buy_size_multiplier)
                )
                
                # But never exceed actual max capacity
                quantity = min(desired_quantity, max_buy_quantity - buy_order_volume)
        ...
        for price in sorted(list(order_depth.buy_orders.keys()), reverse=True):
            # Sell threshold is more conservative in lower sunlight regimes
            if price > implied_ask + take_width + sell_threshold_adj:
                # Scale desired sell quantity based on regime
                desired_quantity = min(
                    abs(order_depth.buy_orders[price]),
                    int(max_sell_quantity * sell_size_multiplier)
                )
                
                # But never exceed actual max capacity
                quantity = min(desired_quantity, max_sell_quantity - sell_order_volume)
    ...
    def macarons_arb_clear(self, position: int, params: Dict, observation: ConversionObservation = None, traderObject: Dict = None) -> int:
        # More strategic with conversions due to limited conversions
        min_conversion_size = params["min_conversion_size"]
        
        # If no position or too small, don't convert
        if abs(position) < min_conversion_size:
            return 0  # Save conversions for when really needed
        
        # If we have observation data, we can be more strategic
        if observation and traderObject:
            sunlight_index = observation.sunlightIndex
            CSI = traderObject.get("dynamic_csi", params["csi_threshold"])
            aggressive_threshold = params["aggressive_threshold"]
            panic_threshold = params["panic_threshold"]
            
            # More aggressive conversion strategy based on sunlight and position
            if position > 0:  # Long position - consider holding during low sunlight
                if sunlight_index < panic_threshold:
                    # In panic mode, hold ALL longs - prices will likely rise more
                    logger.print(f"[MACARONS] HOLDING LONGS in PANIC MODE - no conversion")
                    return 0
                elif sunlight_index < aggressive_threshold:
                    # In aggressive mode, hold most longs, only convert if very large
                    conversion_threshold = min_conversion_size * 3
                    if position >= conversion_threshold:
                        logger.print(f"[MACARONS] Converting EXCESS longs in AGGRESSIVE MODE")
                        return -(position - conversion_threshold)  # Keep some longs
                    else:
                        return 0
                elif sunlight_index < CSI:
                    # In below CSI mode, hold moderate longs, convert if large
                    conversion_threshold = min_conversion_size * 2
                    if position >= conversion_threshold:
                        logger.print(f"[MACARONS] Converting some longs in ALERT MODE")
                        return -(position - conversion_threshold)  # Keep some longs
                    else:
                        return 0
            
            elif position < 0:  # Short position - always try to convert when profitable
                # Always convert shorts, especially during low sunlight
                logger.print(f"[MACARONS] Converting shorts position: {position}")
                return -position
        
        # Default behavior if no observation data or normal conditions
        logger.print(f"[MACARONS] Standard conversion of position: {position}")
        return -position
    ...
    def macarons_arb_make(
        ...
        # Get trading regime based on sunlight levels
        trading_regime = "normal"
        if sunlight_index < panic_threshold:
            trading_regime = "panic"
        elif sunlight_index < aggressive_threshold:
        ...
        # Adjust market making strategy based on trading regime
        if trading_regime == "panic":
            # Maximum aggression - extremely biased to long positions
            logger.print(f"[MACARONS] PANIC MODE - Sunlight critically low at {sunlight_index:.2f}!")
            sunlight_adj = (CSI - sunlight_index) * params["sunlight_factor"] * 0.8
            bid_edge = max(0, edge - 2.0)  # Extremely aggressive bids, almost at market price
            ask_edge = edge + sunlight_adj * 3.0  # Very conservative asks
            
        elif trading_regime == "aggressive":
            # Strong aggression - highly biased to long positions
            logger.print(f"[MACARONS] AGGRESSIVE MODE - Sunlight very low at {sunlight_index:.2f}")
            sunlight_adj = (CSI - sunlight_index) * params["sunlight_factor"] * 0.7
            bid_edge = max(0, edge - 1.5)  # Very aggressive bids
            ask_edge = edge + sunlight_adj * 2.5  # More conservative asks
            
        elif trading_regime == "below_csi":
            # Moderate aggression - biased to long positions
            logger.print(f"[MACARONS] ALERT MODE - Sunlight below CSI at {sunlight_index:.2f}")
            sunlight_adj = (CSI - sunlight_index) * params["sunlight_factor"] * 0.6
            bid_edge = max(0, edge - 1.0)  # Moderately aggressive bids
            ask_edge = edge + sunlight_adj * 2.0  # Moderately conservative asks
            
        # Even in normal conditions, be slightly biased to buy when close to thresholds
        elif sunlight_index < CSI + 2.0:
            # Almost at the threshold, start being more aggressive
            logger.print(f"[MACARONS] WATCHING - Approaching threshold at {sunlight_index:.2f}")
            bid_edge = max(0, edge - 0.5)  # Slightly more aggressive bids
            ask_edge = edge + 1.0  # Slightly more conservative asks
        ...
        if position > 0:  # Long - prefer to sell
            if trading_regime == "panic" or trading_regime == "aggressive":
                # In very low sunlight, be reluctant to sell longs even with large position
                bid_edge += position_factor * 0.5  # Minor adjustment
                # Don't make asks more aggressive in panic mode
            elif trading_regime == "below_csi":
                # In moderately low sunlight, be less eager to sell longs
                bid_edge += position_factor * 1.0
                ask_edge -= position_factor * 0.2  # Small adjustment
            else:
                # In normal conditions, try to reduce long positions
                bid_edge += position_factor * 2.0
                ask_edge -= position_factor * 1.0
        elif position < 0:  # Short - prefer to buy
            # Always be aggressive to cover shorts, especially in low sunlight
            if trading_regime == "panic" or trading_regime == "aggressive":
                # Extremely urgent to cover shorts in low sunlight
                bid_edge -= abs(position_factor) * 2.5
                ask_edge += 0  # Don't adjust ask price
            else:
                # Otherwise normal short covering behavior
                bid_edge -= abs(position_factor) * 1.5
                ask_edge += abs(position_factor) * 2.0
            
        # Set our prices
        bid_price = implied_bid - bid_edge
        ask_price = implied_ask + ask_edge
        
        # Adjust order sizes based on regime
        buy_size_multiplier = 1.0
        sell_size_multiplier = 1.0
        
        if trading_regime == "panic":
            # Almost entirely focus on buying in panic mode
            buy_size_multiplier = 3.0  # Extreme buy size increase - will be capped by position limit
            sell_size_multiplier = 0.2  # Almost stop selling
            logger.print(f"[MACARONS] PANIC MODE - Buy size x3, Sell size x0.2")
        elif trading_regime == "aggressive": 
            buy_size_multiplier = 2.0  # Double buy size
            sell_size_multiplier = 0.4  # Significantly reduce sell size
            logger.print(f"[MACARONS] AGGRESSIVE MODE - Buy size x2, Sell size x0.4")
        elif trading_regime == "below_csi":
            buy_size_multiplier = 1.5  # Increase buy size
            sell_size_multiplier = 0.6  # Reduce sell size
            logger.print(f"[MACARONS] ALERT MODE - Buy size x1.5, Sell size x0.6")
        elif sunlight_index < CSI + 2.0:
            # Slightly adjust when close to threshold
            buy_size_multiplier = 1.2  # Slightly increased buy size
            sell_size_multiplier = 0.9  # Slightly reduced sell size
            
        # Place orders with adjusted sizes
        buy_quantity = min(
            position_limit - (position + buy_order_volume),
            int((position_limit - position) * buy_size_multiplier)
        )
        
        # Make sure we're not leaving tiny quantities on the book
        buy_quantity = max(buy_quantity, 5) if buy_quantity > 0 else 0
        
        if buy_quantity > 0:
            orders.append(Order(Product.MAGNIFICENT_MACARONS, round(bid_price), buy_quantity))
            logger.print(f"[MACARONS] Bidding for {buy_quantity} @ {round(bid_price)}")
    
        sell_quantity = min(
            position_limit + (position - sell_order_volume),
            int((position_limit + position) * sell_size_multiplier)
        )
        
        # Make sure we're not leaving tiny quantities on the book
        sell_quantity = max(sell_quantity, 5) if sell_quantity > 0 else 0
        
        if sell_quantity > 0:
            orders.append(Order(Product.MAGNIFICENT_MACARONS, round(ask_price), -sell_quantity))
            logger.print(f"[MACARONS] Offering {sell_quantity} @ {round(ask_price)}")
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        if "sunlight_history" not in traderObject: traderObject["sunlight_history"] = []
        ...
        if Product.MAGNIFICENT_MACARONS in self.params and Product.MAGNIFICENT_MACARONS in state.order_depths:
                    ...
                    if prod in state.observations.conversionObservations:
                        ...
                        # Market make with dynamic pricing
                        ...
                        # Strategic conversion with full context
                        macarons_conversions = self.macarons_arb_clear(pos, params, observation, traderObject)
                        conversions = macarons_conversions
                        
                        # Combine all orders
                        result[prod] = macarons_take_orders + macarons_make_orders
                        
                        # Determine trading regime for detailed logging
                        CSI = traderObject.get("dynamic_csi", params["csi_threshold"])
                        aggressive_threshold = params["aggressive_threshold"]
                        panic_threshold = params["panic_threshold"]
                        ...
                        if sunlight_index < panic_threshold:
                            ...
                            trading_regime = "ALERT"
                            
                        # Log detailed trading info
                        logger.print(f"[{prod}] Pos: {pos}, Regime: {trading_regime}, Sunlight: {sunlight_index:.2f}, CSI: {CSI:.2f}")
                        logger.print(f"[{prod}] Thresholds - CSI: {CSI:.2f}, Aggressive: {aggressive_threshold:.2f}, Panic: {panic_threshold:.2f}")
                        
                        if trading_regime != "NORMAL":
                            # Log detailed info about price impact
                            sunlight_delta = max(0.1, CSI - sunlight_index)
                            nonlinear_impact = params["sunlight_factor"] * (1 - math.exp(-sunlight_delta))
                            logger.print(f"[{prod}] Sunlight impact: +{nonlinear_impact:.2f} price units ({trading_regime} mode)")
                    else:
