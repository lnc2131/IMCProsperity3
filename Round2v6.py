...
PARAMS = {
    ...
    Product.SQUID_INK: {
        "base_value": 1950, # need to update this to moving average 
...
# DJEMBE basket‑implied mean‑reversion
    Product.DJEMBES: {
        "z_entry": 3.2,          # enter when |z| ≥ 2
        "z_exit": 1.4,           # flatten when |z| ≤ 0.8
        "window": 300,            # rolling window for σ of mis‑pricing
        "pos_lim": 60            # max ± position in DJEMBES
    },

    
    # ── JAMS blended‑FV parameters ─────────────────────────────────────
    Product.JAMS: {
        "static_fv":          6500,   # anchor fair value
        "window_size":        10000,     # rolling‑mid window (ticks)
        "initial_dynamic_fv": 6500,   # start‑up value
        "take_width":         2,      # hit best bid/ask if edge ≥ take_width
        "clear_width":        2,      # clear inventory inside this band
        "volume_limit":       350,    # max net position
        "blend_factor":       0.5    # 0 → 100 % static, 1 → 100 % dynamic
    },
    

# 1. Add to PARAMS dictionary (place this alongside the JAMS parameters)
    Product.CROISSANTS: {
        "static_fv":          4250,   # anchor fair value
        "window_size":        5000,    # rolling‑mid window (ticks)
        "initial_dynamic_fv": 4250,   # start‑up value
        "take_width":         3,      # hit best bid/ask if edge ≥ take_width
        "clear_width":        2,      # clear inventory inside this band
        "volume_limit":       250,    # max net position
        "blend_factor":       0.6     # 0 → 100 % static, 1 → 100 % dynamic
    }
...
class Trader:
    def __init__(self, params = None):
        ...
        self.croiss_params = self.params[Product.CROISSANTS]
        ...
        # ── JAMS blended‑FV state ─────────────────────────────────────
        pj = self.params[Product.JAMS]
        self.jam_window       = pj["window_size"]
        self.jam_blend        = pj["blend_factor"]
        self.jam_take_width   = pj["take_width"]
        self.jam_clear_width  = pj["clear_width"]
        self.jam_pos_limit    = pj["volume_limit"]
        self.jam_dynamic_fv   = pj["initial_dynamic_fv"]
        # we'll use self.price_history[Product.JAMS] as the rolling buffer
        
        
        pc = self.params[Product.CROISSANTS]
        self.croissant_window       = pc["window_size"]
        self.croissant_blend        = pc["blend_factor"]
        self.croissant_take_width   = pc["take_width"]
        self.croissant_clear_width  = pc["clear_width"]
        self.croissant_pos_limit    = pc["volume_limit"]
        self.croissant_dynamic_fv   = pc["initial_dynamic_fv"]

# ───────────────── DJEMBE helpers ──────────────────────────────────────────
    def djembe_fv(self, depths: Dict[str, OrderDepth]) -> float | None:
        """Synthetic fair‑value of ONE Djembe using 2·B1 − 3·B2 = 2·D."""
        req = [Product.PICNIC_BASKET1, Product.PICNIC_BASKET2]
        if any(p not in depths or not depths[p].buy_orders or not depths[p].sell_orders for p in req):
            return None
        b1_mid = self.get_swmid(depths[Product.PICNIC_BASKET1])
        b2_mid = self.get_swmid(depths[Product.PICNIC_BASKET2])
        if b1_mid is None or b2_mid is None:
            return None
        return (2 * b1_mid - 3 * b2_mid) / 2          # price of 1 Djembe
    
    def make_DJEMBE_orders(self, depths: Dict[str, OrderDepth], position: int) -> List[Order]:
        """Mean‑revert DJEMBE toward basket‑implied FV; trades DJEMBE only."""
        orders: List[Order] = []
        if Product.DJEMBES not in depths or not depths[Product.DJEMBES].buy_orders \
           or not depths[Product.DJEMBES].sell_orders:
            return orders
    
        fair = self.djembe_fv(depths)
        if fair is None:
            return orders
    
        dj_depth = depths[Product.DJEMBES]
        best_bid = max(dj_depth.buy_orders.keys())
        best_ask = min(dj_depth.sell_orders.keys())
        bid_vol  = abs(dj_depth.buy_orders[best_bid])
        ask_vol  = abs(dj_depth.sell_orders[best_ask])
        mid      = 0.5 * (best_bid + best_ask)
    
        # rolling window of mis‑pricing
        mis_hist = self.price_history[Product.DJEMBES]
        mis_hist.append(mid - fair)
        if len(mis_hist) > self.params[Product.DJEMBES]["window"]:
            mis_hist.pop(0)
        if len(mis_hist) < self.params[Product.DJEMBES]["window"]:
            return orders
    
        sigma   = np.std(mis_hist) or 1e-9
        z       = (mid - fair) / sigma
        z_in    = self.params[Product.DJEMBES]["z_entry"]
        z_out   = self.params[Product.DJEMBES]["z_exit"]
        pos_lim = self.params[Product.DJEMBES]["pos_lim"]
    
        # target inventory
        if   z >=  z_in:  target = -pos_lim
        elif z <= -z_in:  target =  pos_lim
        elif abs(z) <= z_out: target = 0
        ...
        if delta > 0:  # need to BUY
            qty = min(delta, ask_vol, pos_lim - position)
            if qty > 0:
                orders.append(Order(Product.DJEMBES, best_ask,  qty))
        else:          # need to SELL
            qty = min(-delta, bid_vol, pos_lim + position)
            if qty > 0:
                orders.append(Order(Product.DJEMBES, best_bid, -qty))
    
        logger.print(f"DJEMBE mis={mid-fair:+.1f} z={z:.2f} pos {position}->{target}")
        return orders


     # ──────────────────────────────────────────────────────────────────
    #   JAMS helper functions (blended static + dynamic FV)
    # ──────────────────────────────────────────────────────────────────
    def update_jam_dynamic_fv(self, best_bid: float, best_ask: float) -> float:
        """Rolling average of mid prices over self.jam_window ticks."""
        mid = (best_bid + best_ask) / 2
        buf = self.price_history[Product.JAMS]
        buf.append(mid)
        if len(buf) > self.jam_window:
            buf.pop(0)
        if buf:
            self.jam_dynamic_fv = sum(buf) / len(buf)
        return self.jam_dynamic_fv

    def jam_final_fv(self, static_fv: float) -> float:
        """Blend static & dynamic fair values."""
        return (1 - self.jam_blend) * static_fv + self.jam_blend * self.jam_dynamic_fv

    def make_JAM_orders(self, depth: OrderDepth, fair_value: float, position: int) -> List[Order]:
        orders: List[Order] = []
        if not depth.buy_orders or not depth.sell_orders:
            return orders

        best_ask = min(depth.sell_orders.keys())
        best_ask_qty = -depth.sell_orders[best_ask]
        best_bid = max(depth.buy_orders.keys())
        best_bid_qty =  depth.buy_orders[best_bid]

        # --- take liquidity --------------------------------------------------
        if best_ask <= fair_value - self.jam_take_width:
            qty = min(best_ask_qty, self.jam_pos_limit - position)
            if qty > 0:
                orders.append(Order(Product.JAMS, best_ask, qty))
                logger.print(f"[JAMS] BUY {qty}@{best_ask}")

        if best_bid >= fair_value + self.jam_take_width:
            qty = min(best_bid_qty, self.jam_pos_limit + position)
            if qty > 0:
                orders.append(Order(Product.JAMS, best_bid, -qty))
                logger.print(f"[JAMS] SELL {qty}@{best_bid}")

        # --- clear inventory -------------------------------------------------
        if position > 0:
            near_bids = [p for p in depth.buy_orders if abs(p - fair_value) <= self.jam_clear_width]
            if near_bids:
                p = max(near_bids)
                qty = min(depth.buy_orders[p], position)
                if qty > 0:
                    orders.append(Order(Product.JAMS, p, -qty))
                    logger.print(f"[JAMS] CLEAR SELL {qty}@{p}")
        elif position < 0:
            near_asks = [p for p in depth.sell_orders if abs(p - fair_value) <= self.jam_clear_width]
            if near_asks:
                p = min(near_asks)
                qty = min(-depth.sell_orders[p], -position)
                if qty > 0:
                    orders.append(Order(Product.JAMS, p, qty))
                    logger.print(f"[JAMS] CLEAR BUY {qty}@{p}")
    ...
    def update_croissant_dynamic_fv(self, best_bid: float, best_ask: float) -> float:
        """Rolling average of mid prices over self.croissant_window ticks."""
        mid = (best_bid + best_ask) / 2
        buf = self.price_history[Product.CROISSANTS]
        buf.append(mid)
        if len(buf) > self.croissant_window:
            buf.pop(0)
        if buf:
            self.croissant_dynamic_fv = sum(buf) / len(buf)
        return self.croissant_dynamic_fv

    def croissant_final_fv(self, static_fv: float) -> float:
        """Blend static & dynamic fair values."""
        return (1 - self.croissant_blend) * static_fv + self.croissant_blend * self.croissant_dynamic_fv
    
    def make_CROISSANT_orders(self, depth: OrderDepth, fair_value: float, position: int) -> List[Order]:
        orders: List[Order] = []
        if not depth.buy_orders or not depth.sell_orders:
            return orders
        ...
        # --- take liquidity --------------------------------------------------
        if best_ask <= fair_value - self.croissant_take_width:
            qty = min(best_ask_qty, self.croissant_pos_limit - position)
            if qty > 0:
                orders.append(Order(Product.CROISSANTS, best_ask, qty))
                logger.print(f"[CROISSANTS] BUY {qty}@{best_ask}")
    
        if best_bid >= fair_value + self.croissant_take_width:
            qty = min(best_bid_qty, self.croissant_pos_limit + position)
            if qty > 0:
                orders.append(Order(Product.CROISSANTS, best_bid, -qty))
                logger.print(f"[CROISSANTS] SELL {qty}@{best_bid}")
    
        # --- clear inventory -------------------------------------------------
        if position > 0:
            near_bids = [p for p in depth.buy_orders if abs(p - fair_value) <= self.croissant_clear_width]
            if near_bids:
                p = max(near_bids)
                qty = min(depth.buy_orders[p], position)
                if qty > 0:
                    orders.append(Order(Product.CROISSANTS, p, -qty))
                    logger.print(f"[CROISSANTS] CLEAR SELL {qty}@{p}")
        elif position < 0:
            near_asks = [p for p in depth.sell_orders if abs(p - fair_value) <= self.croissant_clear_width]
            if near_asks:
                p = min(near_asks)
                qty = min(-depth.sell_orders[p], -position)
                if qty > 0:
                    orders.append(Order(Product.CROISSANTS, p, qty))
                    logger.print(f"[CROISSANTS] CLEAR BUY {qty}@{p}")
    ...
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        if Product.CROISSANTS in state.order_depths:
            ...
            if od.buy_orders and od.sell_orders:
                ...
                self.update_croissant_dynamic_fv(best_bid, best_ask)
        
            final_fv = self.croissant_final_fv(static_fv)
            croissant_orders = self.make_CROISSANT_orders(od, final_fv, croissant_pos)
            if croissant_orders:
                result[Product.CROISSANTS] = croissant_orders
        
        # === DJEMBE basket‑implied mean‑reversion ===============================
        if Product.DJEMBES in state.order_depths:
            pos_dj = state.position.get(Product.DJEMBES, 0)
            dj_orders = self.make_DJEMBE_orders(state.order_depths, pos_dj)
            if dj_orders:
                result[Product.DJEMBES] = dj_orders

        
        # === JAMS blended‑FV strategy =======================================
        if Product.JAMS in state.order_depths:
            jam_pos = state.position.get(Product.JAMS, 0)
            od      = state.order_depths[Product.JAMS]

            static_fv = self.params[Product.JAMS]["static_fv"]
            if od.buy_orders and od.sell_orders:
                best_bid = max(od.buy_orders.keys())
                best_ask = min(od.sell_orders.keys())
                self.update_jam_dynamic_fv(best_bid, best_ask)

            final_fv  = self.jam_final_fv(static_fv)
            jam_orders = self.make_JAM_orders(od, final_fv, jam_pos)
            if jam_orders:
